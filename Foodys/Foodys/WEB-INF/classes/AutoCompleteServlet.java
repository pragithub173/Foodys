            ...
            throws IOException, ServletException
	{
		try
		{
			...
			// check if user sent empty string
				if (!searchId.equals(""))
				{
					...
					if (namesAdded)
					{
						...
						response.getWriter().write("<restuarant>" + sb.toString() + "</restuarant>");
					}
					else
					{
						//nothing to show
			...
			if (action.equals("lookup"))
			{
				...
				HashMap<String,Restuarants> data=AjaxUtility.getData();
				if ((searchId != null) && data.containsKey(searchId.trim()))
				{
					request.setAttribute("data",data.get(searchId.trim()));
					RequestDispatcher rd=context.getRequestDispatcher("/RestuarantData");
